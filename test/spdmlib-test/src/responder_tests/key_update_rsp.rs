...
#[test]
fn test_case2_handle_spdm_key_update() {
    let future = async {
        let pcidoe_transport_encap = Arc::new(Mutex::new(PciDoeTransportEncap {}));
        let (config_info, provision_info) = create_info();
        secret::asym_sign::register(SECRET_ASYM_IMPL_INSTANCE.clone());
        ...
        let origin_secret = context.common.session[0].get_application_secret();
        let spdm_message_header = &mut [0u8; 1024];
        let mut writer = Writer::init(spdm_message_header);
        let value = SpdmMessageHeader {
            version: SpdmVersion::SpdmVersion10,
        ...
        let mut writer = Writer::init(key_exchange);
        let value = SpdmKeyUpdateRequestPayload {
            key_update_operation: SpdmKeyUpdateOperation::SpdmVerifyNewKey, // invalid request
            tag: 100u8,
        };
        assert!(value.spdm_encode(&mut context.common, &mut writer).is_ok());
        ...
        let new_secret = context.common.session[0].get_application_secret();
        assert_eq!(
