...
async fn fuzz_handle_spdm_digest(data: Arc<Vec<u8>>) {
    ...
    // -
    {
        ...
        let _ = context.handle_spdm_digest(&data, None, &mut writer);
    }
    // TCD:
    // - id: 0
    // - title: 'Fuzz SPDM handle digest request'
    ...
    // -
    {
        ...
        let mut response_buffer = [0u8; spdmlib::config::MAX_SPDM_MSG_SIZE];
        let mut writer = codec::Writer::init(&mut response_buffer);
        let _ = context.handle_spdm_digest(&data, Some(4294836221), &mut writer);
    }
}
...
fn main() {
    ...
    #[cfg(not(feature = "fuzz"))]
    {
        ...
        if args.len() < 2 {
            ...
            executor::block_on(fuzz_handle_spdm_digest(Arc::new(fuzzdata)));
        } else {
            let path = &args[1];
            let data = std::fs::read(path).expect("read crash file fail");
            executor::block_on(fuzz_handle_spdm_digest(Arc::new(data)));
        }
    }
    #[cfg(feature = "fuzz")]
    afl::fuzz!(|data: &[u8]| {
        executor::block_on(fuzz_handle_spdm_digest(Arc::new(data.to_vec())));
    });
}
