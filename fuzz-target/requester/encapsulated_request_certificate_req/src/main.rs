...
async fn fuzz_encap_handle_get_certificate(fuzzdata: Arc<Vec<u8>>) {
...
fn main() {
    ...
    #[cfg(not(feature = "fuzz"))]
    {
        ...
        if args.len() < 2 {
            ...
            executor::block_on(fuzz_encap_handle_get_certificate(Arc::new(fuzzdata)));
        } else {
            let path = &args[1];
            let data = std::fs::read(path).expect("read crash file fail");
            executor::block_on(fuzz_encap_handle_get_certificate(Arc::new(data)));
        }
    }
    #[cfg(feature = "fuzz")]
    afl::fuzz!(|data: &[u8]| {
